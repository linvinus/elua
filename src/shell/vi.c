...
const char shell_help_vi[] = "[file ..]\n"
  "  [<file>] - file to be edited\n"
  "vi clone from busybox\n";
const char shell_help_summary_vi[] = "text editor";
...
#define CONFIG_FEATURE_VI_MAX_LEN 100
...
static void bb_putchar( u8 data )
{
  platform_uart_send( CON_UART_ID, data );
}

//~ static int bbxmodem_recv( timer_data_type timeout )
//~ {
...
#define text_size      (G_BUSY.text_size     )
#define end            (G_BUSY.end            )
#define dot            (G_BUSY.dot           )
#define reg            (G_BUSY.reg           )

#define vi_setops               (G_BUSY.vi_setops          )
#define editing                 (G_BUSY.editing            )
#define cmd_mode                (G_BUSY.cmd_mode           )
#define modified_count          (G_BUSY.modified_count     )
#define last_modified_count     (G_BUSY.last_modified_count)
#define save_argc               (G_BUSY.save_argc          )
#define cmdcnt                  (G_BUSY.cmdcnt             )
#define rows                    (G_BUSY.rows               )
#define columns                 (G_BUSY.columns            )
#define crow                    (G_BUSY.crow               )
#define ccol                    (G_BUSY.ccol               )
#define offset                  (G_BUSY.offset             )
#define status_buffer           (G_BUSY.status_buffer      )
#define have_status_msg         (G_BUSY.have_status_msg    )
#define last_status_cksum       (G_BUSY.last_status_cksum  )
#define current_filename        (G_BUSY.current_filename   )
#define screen                  (G_BUSY.screen             )
#define screensize              (G_BUSY.screensize         )
#define screenbegin             (G_BUSY.screenbegin        )
#define tabstop                 (G_BUSY.tabstop            )
#define last_forward_char       (G_BUSY.last_forward_char  )
#define erase_char              (G_BUSY.erase_char         )
#define last_input_char         (G_BUSY.last_input_char    )
#if ENABLE_FEATURE_VI_READONLY
#define readonly_mode           (G_BUSY.readonly_mode      )
#else
#define readonly_mode           0
#endif
#define adding2q                (G_BUSY.adding2q           )
#define lmc_len                 (G_BUSY.lmc_len            )
#define ioq                     (G_BUSY.ioq                )
#define ioq_start               (G_BUSY.ioq_start          )
#define my_pid                  (G_BUSY.my_pid             )
#define last_search_pattern     (G_BUSY.last_search_pattern)

#define edit_file__cur_line     (G_BUSY.edit_file__cur_line)
#define refresh__old_offset     (G_BUSY.refresh__old_offset)
#define format_edit_status__tot (G_BUSY.format_edit_status__tot)

#define YDreg          (G_BUSY.YDreg         )
#define Ureg           (G_BUSY.Ureg          )
#define mark           (G_BUSY.mark          )
#define context_start  (G_BUSY.context_start )
#define context_end    (G_BUSY.context_end   )
#define restart        (G_BUSY.restart       )
//~ #define term_orig      (G_BUSY.term_orig     )
//~ #define term_vi        (G_BUSY.term_vi       )
#define initial_cmds   (G_BUSY.initial_cmds  )
#define readbuffer     (G_BUSY.readbuffer    )
#define scr_out_buf    (G_BUSY.scr_out_buf   )
#define last_modifying_cmd  (G_BUSY.last_modifying_cmd )
#define get_input_line__buf (G_BUSY.get_input_line__buf)

#if ENABLE_FEATURE_VI_UNDO
#define undo_stack_tail  (G_BUSY.undo_stack_tail )
# if ENABLE_FEATURE_VI_UNDO_QUEUE
#define undo_queue_state (G_BUSY.undo_queue_state)
#define undo_q           (G_BUSY.undo_q          )
#define undo_queue       (G_BUSY.undo_queue      )
#define undo_queue_spos  (G_BUSY.undo_queue_spos )
# endif
#endif
...
void shell_vi( int argc, char **argv )
{
	int c;
  ...
  memset(ptr_to_globals,'\0',sizeof(globals));
	...
	while ((c = getopt(argc, argv, "hCRH" IF_FEATURE_VI_COLON("c:"))) != -1) {
		...
		case 'c':		// cmd line vi command
			if (*optarg)
				initial_cmds[initial_cmds[0] != NULL] = strndup(optarg, MAX_INPUT_LEN);
			break;
#endif
		case 'H':
			show_help();
...
static int init_text_buffer(char *fn)
{
	...
	free(text);
	text_size = 10240;
	screenbegin = dot = end = text = malloc(text_size);
...
#define cur_line edit_file__cur_line
	...
	rows = 24;
	columns = 80;
	IF_FEATURE_VI_ASK_TERMINAL(G_BUSY.get_rowcol_error =) query_screen_dimensions();
#if ENABLE_FEATURE_VI_ASK_TERMINAL
	if (G_BUSY.get_rowcol_error /* TODO? && no input on stdin */) {
		uint64_t k;
...
static void undo_push(char *src, unsigned int length, uint8_t u_type)	// Add to the undo stack
{
	...
	if (u_type == UNDO_DEL || u_type == UNDO_DEL_CHAIN) {
		...
		undo_entry = malloc(offsetof(struct undo_object, undo_text) + length);
		memcpy(undo_entry->undo_text, src, length);
	} else {
		undo_entry = malloc(sizeof(*undo_entry));
	}
...
//----- IO Routines --------------------------------------------
static int getchar_timeout( timer_data_type timeout )
{
  return platform_uart_recv( CON_UART_ID, CON_TIMER_ID, timeout );
}
...
int64_t read_key(int fd, char *buffer, int timeout)
{
 ...
 start_over:
	...
	if (n == 0) {
		 ...
		 */
    tmpchar=getchar_timeout(timeout);
    if(tmpchar>0){
      buffer[0]=tmpchar;
      n++;//emulate read
	...
	while (*seq != '\0') {
		...
		while (1) {
          ...
          tmpchar=getchar_timeout(50*1000);
          if(tmpchar>0){
            buffer[n]=tmpchar;
            n++;//emulate read
          //~ n = safe_read(fd, buffer, 1);
      ...
      break;
		...
		if ((/*ENABLE_FEATURE_EDITING_ASK_TERMINAL
		 ...
		 && isdigit(buffer[1])
			...
			row = strtoul(buffer + 1, &tmp_end, 10);
			if (*tmp_end != ';' || !isdigit(tmp_end[1]))
				continue;
			col = strtoul(tmp_end + 1, &tmp_end, 10);
			if (*tmp_end != 'R')
				continue;
...
static int readit(void) // read (maybe cursor) key from stdin
{
	...
	fflush_all();
	c = read_key(DM_STDIN_NUM, readbuffer, /*timeout off:*/ -2);
	if (c == -1) { // EOF/error
		go_bottom_and_clear_to_eol();
		cookmode(); // terminal to "cooked"
...
static int file_insert(const char *fn, char *p, int initial)
{
  ...
  fd = open( fn, O_RDONLY, 0 );
	...
	/* Validate file */
	//~ if (fstat(fd, &statbuf) < 0) {//elua not implemented
		//~ status_line_bold_errno(fn);
		//~ goto fi;
	//~ }
	//~ if (!S_ISREG(statbuf.st_mode)) {//elua not implemented
		//~ status_line_bold("'%s' is not a regular file", fn);
		//~ goto fi;
	//~ }
	size = 10240;//INT_MAX;//(statbuf.st_size < INT_MAX ? (int)statbuf.st_size : INT_MAX);
	p += text_hole_make(p, size);
  cnt = read( fd, p, size );
  //~ size=cnt;//elua no way to know file size before read :(
	if (cnt < 0) {
		status_line_bold_errno(fn);
...
static int file_write(char *fn, char *first, char *last)
{
	...
	//~ ftruncate(fd, charcnt); //elua not implemented
	if (charcnt == cnt) {
		// good write
...
#define old_offset refresh__old_offset
	...
	if (ENABLE_FEATURE_VI_WIN_RESIZE IF_FEATURE_VI_ASK_TERMINAL(&& !G_BUSY.get_rowcol_error) ) {
